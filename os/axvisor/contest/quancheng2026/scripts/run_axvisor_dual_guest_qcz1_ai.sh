...
Options:
  ...
  --linux-rt-samples N     Linux guest periodic sample count. Default: 2000.
...
for numeric_value in "${qemu_timeout_seconds}" "${linux_rt_samples}" "${linux_rt_period_ns}" "${linux_stress_workers}" "${linux_stress_seconds}"; do
...
rootfs_image_source="${rootfs_cache}/rootfs-aarch64-alpine.img"
rootfs_registry="${rootfs_cache}/images.toml"
rootfs_registry_url="https://raw.githubusercontent.com/rcore-os/tgosimages/refs/heads/main/registry/v0.0.12.toml"
...
affinity_run_src="${contest_dir}/linux/qc_affinity_run.c"
...
affinity_run_obj="${binary_dir}/qc-affinity-run.o"
affinity_run_bin="${binary_dir}/qc-affinity-run"
...
stop_tcpdump() {
    if [[ -n "${tcpdump_pid}" ]] && kill -0 "${tcpdump_pid}" >/dev/null 2>&1; then
        sudo_cmd pkill -INT -P "${tcpdump_pid}" >/dev/null 2>&1 || true
        if [[ -n "${bridge_dev}" ]]; then
            sudo_cmd pkill -INT -f "tcpdump -eni ${bridge_dev} -vv udp port 4242" >/dev/null 2>&1 || true
        fi
        ...
        if kill -0 "${tcpdump_pid}" >/dev/null 2>&1; then
            if [[ -n "${bridge_dev}" ]]; then
                sudo_cmd pkill -KILL -f "tcpdump -eni ${bridge_dev} -vv udp port 4242" >/dev/null 2>&1 || true
            fi
            sudo_cmd kill -KILL "${tcpdump_pid}" >/dev/null 2>&1 || true
        fi
...
echo "linux_rt_period_ns=${linux_rt_period_ns}"
...
if [[ ! -f "${rootfs_image_source}" ]]; then
    ...
    echo "action=cargo xtask image --registry ${rootfs_registry_url} --download-dir tmp/axbuild/rootfs --extract-dir tmp/axbuild/rootfs pull --arch aarch64"
    (cd "${repo}" && cargo xtask image --registry "${rootfs_registry_url}" --download-dir tmp/axbuild/rootfs --extract-dir tmp/axbuild/rootfs pull --arch aarch64)
fi

if [[ ! -f "${rootfs_image_source}" ]]; then
    echo "missing_required_path=${rootfs_image_source}" >&2
    echo "hint=run 'cargo xtask image --registry ${rootfs_registry_url} --download-dir tmp/axbuild/rootfs --extract-dir tmp/axbuild/rootfs pull --arch aarch64' from the repository root, then retry" >&2
    exit 12
fi
...

{
    ...
    printf 'QC_LINUX_RT_CPU=%s\n' "${linux_rt_cpu}"
    printf 'QC_LINUX_STRESS_CPU=%s\n' "${linux_stress_cpu}"
...
compile_static_aarch64 \
    "${rt_probe_src}" "${rt_probe_obj}" "${rt_probe_bin}" \
    "-DQC_SAMPLES=${linux_rt_samples}" "-DQC_PERIOD_NS=${linux_rt_period_ns}"
compile_static_aarch64 "${affinity_run_src}" "${affinity_run_obj}" "${affinity_run_bin}"

echo "--- static binaries ---"
file "${echo_probe_bin}" "${qcz1_demo_bin}" "${rt_probe_bin}" "${affinity_run_bin}"
sha256sum \
    "${echo_probe_src}" "${echo_probe_bin}" \
    "${qcz1_demo_src}" "${qcz1_demo_bin}" \
    "${rt_probe_src}" "${rt_probe_bin}" \
    "${affinity_run_src}" "${affinity_run_bin}" \
    "${init_normalized}" \
...
write ${affinity_run_bin} /qc-affinity-run
set_inode_field /qc-affinity-run mode 0100755
stat /qc-dual-net.sh
stat /qc-udp-probe
stat /qc-qcz1-demo
stat /qc-rt-probe
stat /qc-affinity-run
EOF
...
debugfs -R "dump /qc-affinity-run ${build_dir}/dump-qc-affinity-run" "${rootfs_img}" >/dev/null 2>&1
...
cmp "${affinity_run_bin}" "${build_dir}/dump-qc-affinity-run"
...
args = [
  ...
  "4",
  "-device",
  "nvme,serial=quancheng-rootfs,drive=disk0,addr=0x2,max_ioqpairs=64,msix_qsize=65",
  "-drive",
  "id=disk0,if=none,format=raw,file=${host_rootfs_img}",
  "-device",
...
linux_cmdline="root=/dev/vda rw init=/qc-dual-net.sh noirqdebug"
if [[ "${linux_quiet}" -eq 1 ]]; then
    linux_cmdline+=" quiet loglevel=3"
fi
...
missing=0
rtos_periodic_ok=0
if grep -aq "QC_RTOS_PERIODIC_RESULT=PASS" "${evidence_dir}/qemu.log"; then
    rtos_periodic_ok=1
elif ! grep -aq "QC_RTOS_PERIODIC_RESULT=FAIL" "${evidence_dir}/qemu.log" \
    && grep -aq "QC_RTOS_PERIODIC_START" "${evidence_dir}/qemu.log" \
    && grep -aq "QC_RTOS_PERIOD_SAMPLES=[1-9]" "${evidence_dir}/qemu.log"; then
    # The RTOS UART and Linux serial streams can interleave. Treat a complete
    # sample stream or any latency summary line as the same runtime proof.
    if grep -aq "QC_RTOS_SAMPLE index=1000" "${evidence_dir}/qemu.log" \
        || grep -aq "QC_RTOS_LATENCY_MEAN_NS=" "${evidence_dir}/qemu.log" \
        || grep -aq "QC_RTOS_LATENCY_P95_NS=" "${evidence_dir}/qemu.log" \
        || grep -aq "QC_RTOS_LATENCY_P99_NS=" "${evidence_dir}/qemu.log" \
        || grep -aq "QC_RTOS_LATENCY_MAX_NS=" "${evidence_dir}/qemu.log"; then
        rtos_periodic_ok=1
        echo "derived_marker=QC_RTOS_PERIODIC_RESULT=PASS_FROM_RTOS_SUMMARY"
    fi
fi
if [[ "${rtos_periodic_ok}" -ne 1 ]]; then
    echo "missing_marker=QC_RTOS_PERIODIC_RESULT=PASS"
    missing=1
fi
