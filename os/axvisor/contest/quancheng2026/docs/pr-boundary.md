...
## Final 2026-08-21 Boundary

The selected final main-PR boundary is intentionally narrow but now includes
the reviewed PCI passthrough-IRQ parser change needed by the submitted Zephyr
e1000 path:
...
The two AxVM files are one cohesive core commit with targeted `1/1` coverage
and a full `296/296` AxVM host-test pass. All other source, scripts, documents
and compact evidence summaries remain under the contest directory. The final
scope audit reports `2` core files, `108` contest files and `0` other files.

## Historical First-Stage Boundary

The earlier safe first-stage boundary was:
...
It remains reviewable independently as the contest-artifact commit.
