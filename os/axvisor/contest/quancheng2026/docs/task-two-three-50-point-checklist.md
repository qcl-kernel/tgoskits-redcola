...
| Quantitative comparison is present | Ready | On the same ten samples, AI/manual mean error is `207/240` and tolerance-band accuracy is `6/10` versus `0/10`; inference time, end-to-end latency and pressure behavior are also recorded. |
...
| Manual baseline comparison with at least two metrics | `4` | Both policies use the same ten samples. Mean absolute error is AI `207` versus manual `240`; samples within the `+/-200` milliunit tolerance band are AI `6/10` versus manual `0/10`. Inference and end-to-end latency are reported separately. | `docs/ai-control-evaluation.md`, `results/task-three-second-metric-latestdev-summary.md`, `docs/demo-video-script.md` | The baseline is a deterministic fixed gain and the tolerance threshold is stated explicitly, making both comparisons reproducible. |
