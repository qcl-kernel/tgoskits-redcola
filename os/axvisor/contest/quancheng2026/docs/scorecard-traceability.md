...
| Compare against fixed manual baseline with at least two metrics | Manual fixed-gain baseline uses `manual_score_milli = 800`; comparison uses timing and control quality. Integrated mean control error is AI `207` versus manual `240`; native smoke is AI `129.003` versus manual `204.640`. | Representative values are in `docs/ai-control-evaluation.md` and `docs/test-report.md`. |
