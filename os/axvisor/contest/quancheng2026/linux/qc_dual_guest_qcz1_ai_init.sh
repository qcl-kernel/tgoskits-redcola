...
start_linux_stress() {
    ...
    echo QC_LINUX_STRESS_CPU="${QC_LINUX_STRESS_CPU}"
    ...
    while [ "${stress_index}" -lt "${QC_LINUX_STRESS_WORKERS}" ]; do
        if [ "${QC_LINUX_STRESS_CPU}" -ge 0 ]; then
            /qc-affinity-run "${QC_LINUX_STRESS_CPU}" /bin/sh -c 'while :; do :; done' &
        else
            (
                while :; do
                    :
...
echo QC_LINUX_RT_CPU="${QC_LINUX_RT_CPU}"
if [ "${QC_LINUX_RT_CPU}" -ge 0 ]; then
    /qc-affinity-run "${QC_LINUX_RT_CPU}" /qc-rt-probe
else
    /qc-rt-probe
fi
rt_status=$?
