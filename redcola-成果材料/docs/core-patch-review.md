...
As of official `dev` `0340ed6bfa36cedd543d48f515e751ccc5a379bf`, the functional
AArch64 physical-timer and GIC work is present upstream through the AxVisor core
history, including merged PR #1770. The current VM schema uses path-based device
passthrough. A small local candidate now extends the generic FDT parser to derive
PCI parent IRQ sources from a host bridge `interrupt-map`; this is required for
the Zephyr e1000 INTx path in the latest dual-guest runtime. The historical patch
notes below remain as traceability for earlier evidence.
...
## Current Candidate: PCI `interrupt-map` Passthrough IRQs
...
- parses PCI host-bridge `interrupt-map` entries and records their parent IRQ
  specifiers in the passthrough route set;
- preserves the host FDT paths until interrupt parsing has completed, then
  expands the path-based passthrough address regions;
- keeps duplicate routes de-duplicated through the existing route set.

Latest validation:

```text
cargo fmt -p axvm -- --check
cargo check -p axvm
cargo check -p axvm --tests
cargo test -p axvm --features host-test --lib
clean AArch64 dual guest: result=PASS
Linux vCPUs: 2
UDP: 20/20
QCZ1: 10/10
AI: 10/10
QEMU filter-dump: 93 packets per guest endpoint
```

Evidence is hash-bound under
`/home/kali/qc-evidence/redcola-latest-dev-clean-pass-20260821`. The dedicated
PCI `interrupt-map` unit test passed, and the repository's host-test mode
completed `296 passed; 0 failed`. A plain host `cargo test -p axvm` is not the
supported gate because it omits the repository's `host-test` feature and may
link architecture-only symbols. The authoritative commands and result are
recorded in `results/axvm-host-test-latestdev0340-summary.txt`.
