...
- virtio-net at `/virtio_mmio@a000000` on the dual-guest link and virtio-blk at
  `/virtio_mmio@a001e00`.
- latest-`dev` passthrough is path-based; AxVisor derives the associated
  interrupt routes from the host FDT instead of carrying hard-coded IRQ IDs in
  the guest TOML.
...
- pCPU `0`, 1 vCPU, and 128 MiB guest memory at `0x90000000`.
- PCI host bridge passthrough at `/pcie@10000000`; the local latest-`dev`
  candidate parses the bridge `interrupt-map` parent IRQ specifiers so the
  e1000 INTx route is assigned to the guest.
...
The contest directory is intentionally separated from AxVisor core changes.
The vTimer/interrupt support was reviewed separately and merged as
`rcore-os/tgoskits#1770` (`024ecca10a4240a84b2c24bed2dc2361a6043d3e`).
On official `dev` `0340ed6b`, the latest dual-guest schema additionally requires
the local PCI host-bridge `interrupt-map` passthrough-IRQ candidate so the
Zephyr e1000 INTx route is derived from the FDT. That candidate is kept as a
separate reviewable core diff and is hash-bound to the latest runtime evidence;
diagnostic-only experiments and generated runtime files are not part of it.
