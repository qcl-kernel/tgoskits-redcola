...
| --- | --- |
| IP-based guest communication | Linux guest `192.0.2.10` communicates with the RTOS guest `192.0.2.20:4242` over IPv4/UDP. |
| Bidirectional application protocol | `CONTROL_SET`, `STATE_REQ`, `ACK`, `STATUS` and `ERROR` frames are defined and implemented by the Linux and Zephyr sides. |
| Reliability over UDP | Sequence validation, timeout/retry, duplicate detection, duplicate ACK and final `STATUS` validation are covered. |
| Plain UDP sanity path | Integrated dual-guest run: `20/20 PASS`. |
| QCZ1 reliable path | Integrated dual-guest run: `10/10 PASS`; native campaign: `200/200` reliable control messages and `40` duplicate ACK responses. |
| Packet-level evidence | TAP/tcpdump run captured `88` packets with `0` kernel drops. |

The detailed runtime summary is in `docs/test-report.md`, and the network
addresses, TAP/bridge isolation and packet-capture setup are described in
